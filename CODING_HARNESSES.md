...
| [Prime Agent](https://github.com/PrimeIntellect-ai/prime-agent) | **Available now** — `polli harness prime on` | Uses Prime Agent's native `models.json` provider support and the Polli skill, and leaves its memories, sessions, and other skills in place. Defaults to `deepseek`. |
...
`on` requires Prime Agent to be installed with its official script: `curl -fsSL https://app.primeintellect.ai/prime-agent/install.sh | sh`. It registers the current compatible Pollinations model catalog in `~/.prime/agent/models.json`, stores a dedicated key in `auth.json`, selects the startup model in `settings.json`, and installs the Polli skill under `skills/polli/`. Choose another default with `--model <id>`. Prime Agent reloads `models.json` every time you open `/model`, so an updated catalog needs no restart. Its memories, sessions, and other skills are left alone, and `off` removes only the Pollinations entries.
